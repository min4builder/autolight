...
{-# INLINEABLE gradient #-}
gradient :: (Default a, Num a, Source r Ix1 a, Unbox a) => Focused r Dim2 a -> Focused (MResult Matrix) Dim2 (a, a)
gradient = extend $ \img ->
    let v = extract img in (get img (1, 0) - v, get img (0, 1) - v)

{-# INLINEABLE gradient' #-}
gradient' :: (Default a, Num a, Source r Ix1 a, Unbox a) => Matrix r Dim2 a -> Matrix (MResult Matrix) Dim2 (a, a)
...
gameOfLife :: Source r Ix1 Bool => Focused r Dim2 Bool -> Focused (MResult Matrix) Dim2 Bool
gameOfLife = extend $ \img ->
    let n = sum [ if get img (x, y) then 1 else 0 | x <- [-1 .. 1], y <- [-1 .. 1], (x, y) /= (0, 0) ] :: Int in
        n == 3 || (extract img && n == 2)

{-# INLINEABLE gameOfLife' #-}
gameOfLife' :: Load r Ix1 Bool => Matrix r Dim2 Bool -> Matrix (MResult Matrix) Dim2 Bool
