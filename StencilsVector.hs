...
{-# INLINEABLE gradient #-}
gradient :: (Default a, Num a, Unbox a) => Focused r Dim2 a -> Focused (MResult Matrix) Dim2 (a, a)
gradient = extend $ \img ->
    let v = extract img in (get img (1, 0) - v, get img (0, 1) - v)

{-# INLINEABLE gradient' #-}
gradient' :: (Default a, Num a, Unbox a) => Matrix r Dim2 a -> Matrix (MResult Matrix) Dim2 (a, a)
