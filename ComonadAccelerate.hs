...
zipWith :: (Elt a, Elt b, Elt c, Shape sh) => (Exp a -> Exp b -> Exp c) -> Focused r sh a -> Focused s sh b -> Focused MResult sh c
zipWith f (Focused a _) (Focused b c) = Focused (mzipWith f a b) c

map :: (Elt a, Elt b) => (Exp a -> Exp b) -> Focused r sh a -> Focused MResult sh b
...
extend :: (Elt a, Elt b, Elt sh, Shape sh, Shape (Exp sh), SInt sh ~ Int, SInt (Exp sh) ~ Exp Int) => (Focused MNormal sh a -> Exp b) -> Focused r sh a -> Focused MResult sh b
