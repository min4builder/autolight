...
zipWith :: (Eq sh, Source r Ix1 a, Source s Ix1 b) => (a -> b -> c) -> Focused r sh a -> Focused s sh b -> Focused (MResult Matrix) sh c
zipWith f (Focused a _) (Focused b c) = Focused (mzipWith f a b) c

map :: Source r Ix1 a => (a -> b) -> Focused r sh a -> Focused (MResult Matrix) sh b
map f (Focused mat c) = Focused (mmap f mat) c

extract :: (Shape sh, Source r Ix1 a) => Focused r sh a -> a
extract (Focused mat p) = mindex mat p

extend :: (Shape sh, Source r Ix1 a, Unbox a) => (Focused (MNormal Matrix) sh a -> b) -> Focused r sh a -> Focused (MResult Matrix) sh b
extend f (Focused mat p) = Focused (mrun (\mat p -> f $ Focused mat p) mat) p

index :: (Shape sh, Source r Ix1 a) => Focused r sh a -> sh -> a
