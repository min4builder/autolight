...
zipWith :: (Eq sh, Source r a, Source s b) => (a -> b -> c) -> Focused r sh a -> Focused s sh b -> Focused (MResult Matrix) sh c
zipWith f (Focused a _) (Focused b c) = Focused (mzipWith f a b) c

map :: Source r a => (a -> b) -> Focused r sh a -> Focused (MResult Matrix) sh b
map f (Focused mat c) = Focused (mmap f mat) c

extract :: (Shape sh, Source r a) => Focused r sh a -> a
extract (Focused mat p) = mindex mat p

extend :: (Evaluator r, Shape sh, Source r a, Unbox a) => (Focused (MNormal Matrix) sh a -> b) -> Focused r sh a -> Focused (MResult Matrix) sh b
